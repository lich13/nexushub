...
- New features must be implemented in this order by default: core/shared use-case/DTO/plan, then WebUI query/domain/runtime contracts, then thin Linux RPC and Tauri invoke adapters. Do not add one-off Linux-only or Tauri-only business contracts unless a capability or host-surface difference is explicitly required.
...
`bash scripts/package-linux-tauri-x86_64.sh` is canonical only on Linux x86_64 and must restore `src-tauri/resources/nexushubd` to the tracked placeholder after packaging.
