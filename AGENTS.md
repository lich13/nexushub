...
- Grok Build reads native session history. Only narrow native rename and confirmed session-directory deletion are supported; never start a task or delete a workspace/worktree.
- Probe is now an internal NexusHub replacement path for the cloud `codex-sentinel-server` runtime. Keep it observable and maintenance oriented: no hidden desktop control, no automatic replies or turn startup, no arbitrary shell, and no direct destructive deletion endpoint outside the existing dry-run plus confirmation model.

## Architecture Boundaries

- Shared thread/Grok, settings, Probe, security, cleanup, job, and update contracts must enter core through `nexushub_core::services::use_cases::NexusHubUseCases`.
...
- Before adding or changing a shared action, run `node scripts/contract-next-action-checklist.mjs <action-id>` or run it without an action id for the global checklist. New features must be implemented in this order by default: update `contracts/nexushub-contract.json`, then core/shared use-case/DTO/plan, then WebUI query/domain/runtime contracts, then thin Linux RPC and Tauri invoke adapters. Do not add one-off Linux-only or Tauri-only business contracts unless a capability or host-surface difference is explicitly required and documented in the contract registry.
...
- Keep `contracts/nexushub-contract.schema.json` in sync with `contracts/nexushub-contract.json`. The schema is the lightweight static boundary for the registry shape; do not replace it with runtime codegen unless a later Goal explicitly chooses that tradeoff. Shared and transport actions must declare `dtoOwner`, `requestDto`, and `responseDto`; those names must be mirrored in the core DTO marker and WebUI DTO marker so camelCase/snake_case request and response drift is caught before release.
...
- Linux HTTP API entry code must stay split by domain under `crates/nexushub-webd/src/api/`; do not re-inline auth, Probe, security, cleanup, job, system/update, or Grok handlers back into `api.rs`.
...
- Desktop LAN WebUI is retired. Keep only embedded Tauri and the fixed Hook/monitor helper. Never restart historical jobs/followups from a read path; preserve their DB rows without execution or fallback.
- Automatic notifications require confirmed main-task identity. Restricted Goal recovery is internal-only, separately configurable, and never starts a turn. Unknown/internal identities are suppressed before mutation or notification.
...
- macOS Tauri, Linux Tauri, and Linux server WebUI share the same `webui` visual vocabulary. Navigation labels, core panel titles, action copy, disabled states and cleanup dry-run gating belong in shared visual/domain contracts; host differences must come only from `SystemCapabilities`, host surface, host policy, or runtime transport.
