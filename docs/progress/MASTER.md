...
> **Mode**: V0.1.122_CC_SWITCH_USE_CASE_ADAPTER_CLOSURE
...
**Active Phase**: v0.1.122 cc-switch use-case adapter closure<br>
**Active Task**: `v0.1.122` keeps the hard-deleted REST/compat command surface intact and routes the remaining shared business transactions through core use-case facades, Linux `linux_adapter`, thin Tauri commands, and WebUI query/view-model helpers so Linux WebUI remains a Linux-only host capability while macOS stays native Tauri.
...
| 2026-06-23 | v0.1.122 cc-switch use-case adapter closure | L | P/R pending | 0 | Bumps workspace/package/Tauri versions to `0.1.122`; adds application-level use-case facade wiring, moves Linux executor adapter calls into `linux_adapter`, keeps Tauri commands as typed adapters, adds `csrf` capability coverage, and moves realtime/failure/action view-model logic behind WebUI query/domain helpers while preserving Linux-only WebUI boundaries and keeping `CLAUDE.md` intentionally absent. |
...
git ls-remote --tags origin refs/tags/v0.1.122
```
...
| 2026-06-23 | v0.1.122-cc-switch-use-case-adapter-closure | Closes the next adapter gap by introducing the shared `NexusHubUseCases` facade, routing Linux RPC executor work through `linux_adapter`, keeping macOS typed commands on core plans, adding `csrf` capability parity, and moving App realtime/failure/action view-model logic behind query/domain helpers. |
