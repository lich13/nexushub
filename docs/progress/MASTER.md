...
> **Mode**: V0.1.123_CC_SWITCH_SINGLE_USE_CASE_CLOSURE
...
**Active Phase**: v0.1.123 cc-switch single use-case closure<br>
**Active Task**: `v0.1.123` keeps the hard-deleted REST/compat command surface intact, moves remaining shared transaction planning into core use-case facades, narrows Linux RPC and macOS Tauri to thin host adapters, and keeps WebUI transport/query/view-model boundaries explicit so Linux WebUI remains a Linux-only host capability while macOS stays native Tauri.
...
git ls-remote --tags origin refs/tags/v0.1.123
```
...
| 2026-06-24 | v0.1.123-cc-switch-single-use-case-closure | Closes the remaining audited single-use-case gaps by splitting Linux RPC payload/surface modules, keeping Tauri overview free of command and desktop business helper dependencies, routing thread home/list through a native service adapter, and hiding raw runtime transport behind frontend domain/query boundaries. |
