...
> **Mode**: V0.1.115_CC_SWITCH_UNIFIED_COMMAND_CLOSURE
...
**Active Phase**: v0.1.115 cc-switch unified command closure<br>
**Active Task**: `v0.1.115` closes the remaining cc-switch alignment gap found after `v0.1.114`: macOS Tauri no longer registers the Linux-only `updates.prune` command surface, while Linux RPC keeps update backup pruning as a WebUI-only host capability. `CLAUDE.md` remains intentionally absent.
...
adaptive:
  mode: V0.1.115_CC_SWITCH_UNIFIED_COMMAND_CLOSURE
  strategy: "cc-switch style shared services with thin Linux WebUI and macOS Tauri adapters; no Linux-only prune command registered on macOS"
  phases:
    phase_1:
      drift_score: 0
...
| 2026-06-21 | v0.1.115 cc-switch macOS prune surface closure | S | P/R pending | 0 | Bumps workspace/package/Tauri versions to `0.1.115`; removes the macOS Tauri `updates.prune` command registration entirely, keeps Linux update backup pruning as a Linux WebUI-only RPC capability, strengthens the Tauri handler guard, and keeps `CLAUDE.md` intentionally absent. |
...
git ls-remote --tags origin refs/tags/v0.1.115
```
...
| macOS ARM64 | Tauri App bundle wrapping shared `webui` | native app process | `~/Library/Application Support/NexusHub`, `~/Library/Application Support/NexusHub/bin/nexushubd`, `~/Library/Logs/NexusHub` | `open -a NexusHub`, no Web login/API admin setup, no Linux update prune command registration, shared typed Probe/Update commands, Codex/Probe/Goal/cleanup smoke, helper sync check, log tail, DMG/tarball `.sha256`, signed updater `.sig`, `latest.json`, Tauri updater `Check` shows the latest signed version in the shared update card and Job History |
...
| 2026-06-21 | v0.1.115-cc-switch-macos-prune-surface-closure | Closes the final macOS command-surface leak by removing `updates.prune` from Tauri registration while keeping Linux update pruning available only through Linux WebUI RPC capability gates; `CLAUDE.md` remains intentionally absent. |
