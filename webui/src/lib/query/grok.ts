...
export function useGrokSessions(q: string) {
  return useQuery({ queryKey: [...grokQueryKeys.list, q], queryFn: () => listGrokSessions({ q, limit: 100 }), refetchInterval: 30000 });
}
export function useGrokDetail(id?: string) {
  return useQuery({ queryKey: ["grok", "session", id], queryFn: () => getGrokSession(id!), enabled: Boolean(id), refetchInterval: 15000 });
}
